...
`hashavatar.app` is the interactive demo website for deterministic, procedural
avatars generated by the [`hashavatar`](https://crates.io/crates/hashavatar)
Rust crate. The site also exposes stable image URLs so the generated avatars can
be tested in real integrations.

This repository is a deployable application, not a reusable Rust crate. Its
Cargo package is marked `publish = false` and must not be published to
crates.io. The reusable renderer is maintained and published separately as
`hashavatar`.
...
The current website version is `1.3.0`.
...
- Avatar families from `hashavatar 1.3.0`: `cat`, `dog`, `robot`, `fox`,
...
| Renderer crate | crates.io `hashavatar 1.3.0` |
...
- local Podman smoke tests for the Wolfi image when
  `HASHAVATAR_WEBSITE_GATE_PODMAN=1` is set
...
```text
ghcr.io/valkyoth/hashavatar-website:<version>
ghcr.io/valkyoth/hashavatar-website:<version>-wolfi
```
